...
def main():
    ...
    parser.add_argument('--momentum', default=0.09, type=float, help='Momentum for batch normalization.')
    ...
    sys.stdout = Logger(f'./Logs/BinaryClassification/SqueezeNet/{file_name}.log')
    ...
    y = Conv2D(kernel_size=7, filters=96, strides=2, padding='same', activation=activation)(y)
    y = MaxPool2D(pool_size=3, strides=2, padding='same')(y)
    ...
    y = Conv2D(filters=2, kernel_size=1)(y)
    y = GlobalAvgPool2D()(y)
    ...
    if not data_augmentation:       
        ...
        model.fit(
            ...
            epochs=epochs,
            validation_data=([np.asarray(X_valid)], y_valid), class_weight=class_weights,
            shuffle=True)
